...
class OptProblem(object):
    ...
    def opt_logistic(self, selected, w0):
        nbr_feats = len(selected) + 1
        X_selected = np.hstack([np.ones((self.X.shape[0], 1)),self.X[:, selected]])
        CS_inv = np.zeros((nbr_feats, nbr_feats))
        CS_inv[0,0] = 1
        CS_inv[1:, 1:] = np.linalg.inv(self.C[selected[:,np.newaxis], selected])
        w = np.zeros(nbr_feats)
        w[:len(w0)] = w0
        ...
        while True:
          # predict
          dot_Xw = X_selected.dot(w)
          exp_dot_Xw = np.exp(dot_Xw)
          ...
          if is_first:
            ...
            is_first = False
          elif np.abs(last_objective - objective) / np.abs(objective) < 1e-4:
            return w, objective
          last_objective = objective

          # update  
          res = (Y_hat - self.Y) / self.Y.shape[0]
          BS = np.dot(res, X_selected) + self.l2_lam * np.hstack([0, w[1:]])
          delta_w = CS_inv.dot(BS)
          w = w - delta_w
          nbr_iter += 1
    ...
    def omp_select_groups_bC(self, selected, mask, w, costs, groups, noinv=False):
        ...
        for g in np.unique(groups):
            ...
            bG = b_res[sel_g]
            CG = self.C_no_regul[sel_g[:,np.newaxis], sel_g]
    ...
    def omp_select_groups(self, selected, mask, w, costs, groups, logistic=False):
        selected_feats = [np.array([], dtype=np.int)]
        selected_feats += [np.nonzero(groups == g)[0] for g in selected]
        selected_feats = np.hstack(selected_feats)
...
def all_results_groups_p(problem, K=None, costs=None, groups=None, optimal=None, release=False, do_FR=True, do_logistic=False):
    results = []
    names = []
...
def all_results_bC(b, C_no_regul, l2_lam, X=None, Y=None, K=None, costs=None, groups=None, optimal=None, release=False, do_FR=True, do_logistic=False, **opt_args):
    if X==None:
...
#          g, grad = problem.omp_select_groups_glm(selected_groups[:k], mask, last_grad, costs, groups, 
        ...
        if method == 'logistic':
          best_w, best_score = problem.opt_logistic(best_set, last_w)
        else:
          best_w = problem.opt(best_set)
